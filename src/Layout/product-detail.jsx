...
function ProductDetail({ productdetail, rating, reviews, tags, images }) {
  ...
  const onlyLeft = () => {
    if (productdetail.stock <= 10 && productdetail.stock >=1) {
      return <Chip label={`Hurry up only ${productdetail.stock} Left`} color='warning' className='font-semibold animate__animated animate__shakeX animate__delay-2s ' variant="outlined" />
    } else if(productdetail.stock == 0){
      return <Chip label={`out of stock!!!`} color='error' variant="outlined" />
    }else{
      return <Chip label={`${productdetail.stock} Left`} color='primary' variant="outlined" />
    }
  }
  ...
  const addTocart = (productdetail) => {
    ...
    if (user) {
      return localStorage.setItem(`cart product`, JSON.stringify(productdetail))
    }
    else {
      return Swal.fire({
        icon: "error",
  ...
  return (
    <div key={productdetail.id} className='mx-auto max-w-2xl px-4 sm:px-6 lg:max-w-7xl lg:px-8'>
      <div className='grid grid-cols-1 items-center lg:grid-cols-2 gap-1'>
        ...
        <div className=' rounded-xl  h-3/4 hover:shadow-inner p-3  hover:overflow-scroll hover:overflow-x-auto transition-all duration-1000 overflow-hidden
      ...
      [&::-webkit-scrollbar-thumb]:bg-gray-300'>
          ...
          <div className='my-3'>
            <span className='sm:text-3xl text-xl flex gap-2 font-semibold'>${calculateDiscountedPrice(productdetail)}
              <span className='text-gray-600 line-through opacity-85'>{productdetail.price}</span>
              <span className='text-green-600 font-semibold'>{productdetail.discountPercentage}% off</span>
            </span>
          ...
          <div>
            <p className='my-5'><span className='my-3 font-semibold text-xl'>About:</span> <br /> <span> {productdetail.description}</span> </p>
          </div>
          ...
          <div>
            <b>Tags:</b>
            {tags.map((item, index) => {
              return <Chip key={index} className='mx-2' label={item} size="small" icon={<SellIcon />} />
            })}
          </div>
      ...
      <div className='bg-gray-200 rounded py-2 px-3'>
        ...
        <div className='flex  overflow-scroll overflow-y-auto transition-all duration-1000 
